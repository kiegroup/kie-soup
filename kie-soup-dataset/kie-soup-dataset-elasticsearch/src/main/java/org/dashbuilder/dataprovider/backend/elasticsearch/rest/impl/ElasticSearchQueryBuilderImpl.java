...
public class ElasticSearchQueryBuilderImpl implements ElasticSearchQueryBuilder<ElasticSearchQueryBuilderImpl> {
    ...
    public Query build() {
        ...
        for (DataSetFilter filter : filters) {
            Query subQuery = build(filter.getColumnFilterList(),
                                   Operator.AND);
            if (subQuery == null) {
                continue;
            }
        ...
        Query result = joinQueriesAndFilters(queries,
                                             Operator.AND);
        ...
        if (result != null && isFilter(result)) {
            ...
            filtered.setParam(Query.Parameter.QUERY.name(),
                              matchAll);
            filtered.setParam(Query.Parameter.FILTER.name(),
                              result);
            return filtered;
        }
        return result;
    ...
    private List<Query> buildGroupIntervalQuery(DataSetGroup group) {
        ...
        for (Interval interval : intervals) {
            ...
            if (isLabelCol) {
                String filterValue = valueTypeMapper.formatLabel(def,
                                                                 sourceId,
                                                                 interval.getName());
                _result = new Query(sourceId,
                                    Query.Type.TERM);
                _result.setParam(Query.Parameter.VALUE.name(),
                                 filterValue);
                ...
                Object value0 = isNumericCol ?
                        valueTypeMapper.formatNumeric(def,
                                                      sourceId,
                                                      (Number) minValue) :
                        valueTypeMapper.formatDate(def,
                                                   sourceId,
                                                   (Date) minValue);
                ...
                _result = new Query(sourceId,
                                    Query.Type.RANGE);
                _result.setParam(Query.Parameter.GT.name(),
                                 value0);
                _result.setParam(Query.Parameter.LT.name(),
                                 value1);
            }
    ...
    private String formatValue(ElasticSearchDataSetDef definition,
        ...
        boolean isTextCol = ColumnType.TEXT.equals(columnType);
        if (isTextCol) {
            return valueTypeMapper.formatText(definition,
                                              columnId,
                                              value != null ? value.toString() : null);
        } else if (isLabelCol) {
            return valueTypeMapper.formatLabel(definition,
                                               columnId,
                                               value != null ? value.toString() : null);
        } else if (isDateCol) {
            return valueTypeMapper.formatDate(definition,
                                              columnId,
                                              (Date) value);
        } else if (isNumericCol) {
            return valueTypeMapper.formatNumeric(definition,
                                                 columnId,
                                                 (Number) value);
        }
    ...
    private Query joinQueriesAndFilters(List<Query> queries,
                                        Operator operator) {
        ...
        if (!existFilters) {
            ...
            if (onlyOneQuery && !operator.equals(Operator.NOT)) {
                ...
                result.setParam(Query.Parameter.FILTER.name(),
                                queries.get(0));
            } else {
                // Multiple filters.
                result = new Query(filterOperator);
                result.setParam(Query.Parameter.FILTERS.name(),
                                queries);
            }
            ...
            if (subFilters.size() == 1) {
                ...
                filter.setParam(Query.Parameter.FILTERS.name(),
                                subFilters);
            }
            ...
            if (subQueries.size() == 1) {
                ...
                booleanQuery.setParam(boolType,
                                      subQueries);
            }
            ...
            if (isAndOp) {
                ...
                result.setParam(Query.Parameter.QUERY.name(),
                                booleanQuery);
                result.setParam(Query.Parameter.FILTER.name(),
                                filter);
            } else {
     ...
     */
    private Query build(List<ColumnFilter> filterList,
                        Operator operator) {
        if (filterList == null) {
            return null;
        }
        ...
        for (ColumnFilter filter : filterList) {
            ...
            if (filter instanceof CoreFunctionFilter) {
                result = buildColumnCoreFunctionFilter((CoreFunctionFilter) filter,
                                                       metadata);
            }
            // Logical expressions.
                ...
                if (LogicalExprType.AND.equals(type)) {
                    result = buildLogicalExpressionFilter(f,
                                                          Operator.AND);
        ...
        return joinQueriesAndFilters(results,
                                     operator);
    }

    protected Query buildLogicalExpressionFilter(LogicalExprFilter filter,
                                                 Operator operator) {
        if (filter == null) {
            return null;
        }
    ...
    protected Query buildColumnCoreFunctionFilter(CoreFunctionFilter filter,
        ...
        if (CoreFunctionType.IS_NULL.equals(type)) {
            ...
            if (ColumnType.LABEL.equals(columnType)) {
                Query resultMatch = buildTermOrTermsFilter(def,
                                                           columnId,
                                                           params);
                result = new Query(columnId,
                                   Query.Type.NOT);
                ...
                Query resultMatch = buildBooleanMatchQuery(def,
                                                           columnId,
                                                           params);
                result = new Query(columnId,
                                   Query.Type.BOOL);
                result.setParam(Query.Parameter.MUST_NOT.name(),
                                asList(resultMatch));
            }
        } else if (CoreFunctionType.LIKE_TO.equals(type)) {

            Object value = formatValue(def,
                                       columnId,
                                       params.get(0));
            ...
            if (value != null) {
                ...
                result.setParam(Query.Parameter.DEFAULT_FIELD.name(),
                                columnId);
                result.setParam(Query.Parameter.DEFAULT_OPERATOR.name(),
                                "AND");
                result.setParam(Query.Parameter.QUERY.name(),
                                pattern);
                result.setParam(Query.Parameter.LOWERCASE_EXPANDED_TERMS.name(),
            ...
            result = new Query(columnId,
                               Query.Type.RANGE);
            result.setParam(Query.Parameter.LT.name(),
                            value);
            ...
            result = new Query(columnId,
                               Query.Type.RANGE);
            result.setParam(Query.Parameter.LTE.name(),
                            value);
            ...
            result = new Query(columnId,
                               Query.Type.RANGE);
            result.setParam(Query.Parameter.GT.name(),
                            value);
            ...
            result = new Query(columnId,
                               Query.Type.RANGE);
            result.setParam(Query.Parameter.GTE.name(),
                            value);
        } else if (CoreFunctionType.BETWEEN.equals(type)) {

            Object value0 = formatValue(def,
                                        columnId,
                                        params.get(0));
            Object value1 = formatValue(def,
                                        columnId,
                                        params.get(1));
            result = new Query(columnId,
                               Query.Type.RANGE);
            ...
            if (timeFrame != null) {
                ...
                String pastRaw = valueTypeMapper.formatDate(def,
                                                            columnId,
                                                            past);
                String futureRaw = valueTypeMapper.formatDate(def,
                                                              columnId,
                                                              future);
                result = new Query(columnId,
                                   Query.Type.RANGE);
    ...
    protected Query buildBooleanMatchQuery(ElasticSearchDataSetDef def,
                                           String columnId,
                                           List params) {
        Query result = new Query(columnId,
                                 Query.Type.MATCH);
        StringBuilder terms = new StringBuilder();
        String _pre = params.size() == 1 ? "" : " ";
        for (Object param : params) {
            String paramStr = formatValue(def,
                                          columnId,
                                          param);
            terms.append(_pre).append(paramStr);
        }
        if (params.size() > 1) {
            result.setParam(Query.Parameter.OPERATOR.name(),
                            "or");
        }
        result.setParam(Query.Parameter.VALUE.name(),
                        terms.toString());
        return result;
    }

    protected Query buildTermOrTermsFilter(ElasticSearchDataSetDef def,
                                           String columnId,
                                           List params) {
        Query result;
        Object value;
